...
interface RecapFilter {
  startDate?: string;
  endDate?: string;
  area?: string; // 'ALL', 'TRANSIT', or specific line name
}

export async function getOpnameRecapPivot(filters: RecapFilter) {
  ...
  const kamusMap: Record<string, { oracleCode: string | null }> = {};
  kamusAll.forEach(k => {
  ...
  bomComponents.forEach(b => { if (b.materialName && b.uom) uomMap[b.materialName] = b.uom; });
  ...
  const pivot: Record<string, {
    ...
    uom: string;
    lines: Record<string, { qtyBook: number; qtyPhysical: number }>;
    transit: { qtyBook: number; qtyPhysical: number };
  }> = {};

  const allLineNames    = new Set<string>();
  // Track which materials were opname'd in the selected period
  const opnamedMaterials = new Set<string>();

  const getOrCreateMaterial = (matName: string) => {
    if (!pivot[matName]) {
      pivot[matName] = {
  ...
  if (shouldFetchLine) {
    const lineWhere: any = {};
    if (hasDates) lineWhere.date = dateFilter;
    if (area && area !== 'ALL' && area !== 'TRANSIT' && area !== 'ALL_LINES') lineWhere.line = area;
    ...
    for (const op of prodOpnames) {
      ...
      for (const item of op.items) {
        ...
        if (!mat.lines[lineName]) mat.lines[lineName] = { qtyBook: 0, qtyPhysical: 0 };
  ...
  if (shouldFetchTransit) {
    ...
    if (hasDates) transitWhere.date = dateFilter;
  ...
  const rows = Object.entries(pivot).map(([materialName, data]) => {
    ...
    const lineValues: Record<string, { qtyBook: number; qtyPhysical: number }> = {};
  ...
  const latestBookQty: Record<string, number> = {};

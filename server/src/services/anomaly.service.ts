...
export async function getUnscannedTransit(anomalyThresholdHours: number = 3): Promise<AnomalyResult[]> {
  const now = new Date();
  ...
  for (const item of transitItems) {
    ...
    if (waitingHours >= anomalyThresholdHours) {
      ...
      let shiftVal = 3;
      if (hours >= 7 && hours < 15) shiftVal = 1;
      else if (hours >= 15 && hours < 23) shiftVal = 2;

      // Upsert into AnomalyLog
      try {
        await (prisma as any).anomalyLog.upsert({
          where: {
            barcode_status: {
  ...
  try {
    ...
    await (prisma as any).anomalyLog.updateMany({
      where: {

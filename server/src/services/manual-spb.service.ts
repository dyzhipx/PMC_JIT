 ...
 */
async function generateSpbNumber(): Promise<string> {
 ...
 */
export async function createManualSpb(
  requestedBy: string,
  reason: string,
  items: Array<{ materialName: string; qtyPallets: number; qtyPcs?: number; targetBlockRowId?: string }>,
  targetDate?: string,
  targetShift?: number
) {
 ...
 */
export async function getManualSpbs(status?: string, page: number = 1, limit: number = 50) {
  const where: any = {};
 ...
 */
export async function getManualSpbById(id: string) {
 ...
 */
export async function processSpbItem(
  itemId: string,
  barcode: string,
  pcs: number,
  supplier: string,
  ...
  return db.$transaction(async (tx: any) => {
    ...
    if (!item) throw new Error(`Item SPB ID ${itemId} tidak ditemukan`);
    if (item.status === "completed") throw new Error("Item ini sudah selesai (terima di Transit)");
    if (item.scannedPallets >= item.qtyPallets) throw new Error("Kuota pengiriman (scan) untuk item ini sudah terpenuhi");
    ...
    if (barcode && barcode !== "-") {
      ...
      if (existingScan) throw new Error(`Barcode ${barcode} sudah pernah diproses di SPB Manual ini/lainnya`);
      
      const existingUsed = await tx.usedBarcode.findUnique({ where: { barcode } });
      if (existingUsed) throw new Error(`Barcode ${barcode} sudah pernah digunakan di sistem (Duplikasi)`);
    }
 ...
 */
export async function receiveSpbScan(barcode: string, actualPcs: number) {
  return db.$transaction(async (tx: any) => {
    ...
    if (!scan) return null; // Not a manual SPB scan
    if (scan.status === "received") throw new Error(`Barcode ${barcode} sudah pernah diterima di Transit`);
    ...
    await transitService.receiveToTransit(
      scan.item.materialName,
      1,
      barcode,
      actualPcs,
      `SPB Manual: ${scan.item.spb.spbNumber}`,
      scan.targetBlockRowId || undefined,
      scan.supplier || "-",
      tx
    );
    ...
    const allComplete = allItems.every((i: any) => {
      if (i.id === item.id) return isItemComplete;
 ...
 */
export async function deleteManualSpb(id: string) {
  const spb = await db.manualSpb.findUnique({
    where: { id },
    include: { items: true },
  });

  if (!spb) return { success: false, message: "SPB tidak ditemukan" };

  const hasScanned = spb.items.some((i: any) => i.scannedPallets > 0);
  if (hasScanned) return { success: false, message: "Tidak dapat menghapus SPB yang sudah diproses" };
 ...
 */
export async function getManualSpbScanByBarcode(barcode: string) {

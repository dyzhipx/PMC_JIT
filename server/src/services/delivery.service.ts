...
//  DELIVERY LIFECYCLE
// ═══════════════════════════════════════════
export async function getOrCreateDelivery(date: string, shiftKey: string, slotId: number, initialItems: Array<{material: string, required: number}> = []) {
...
export async function addDeliveryItem(deliveryId: string, materialName: string, requiredPallets: number) {
...
export async function scanDeliveryItem(deliveryId: string, material: string, barcode: string, qtyPallet: number, pcs: number, supplier: string, targetBlockRowId?: string) {
  ...
  return db.$transaction(async (tx: any) => {
    ...
    if (delivery.status !== "preparing") return { success: false, message: "Status delivery tidak valid untuk scan" };

    // Find the item
    const item = delivery.items.find((i: any) => i.materialName === material);
    if (!item) return { success: false, message: `Material ${material} tidak dibutuhkan pada delivery ini` };

    // Check duplicate barcode across all deliveries
    if (barcode && barcode !== "-") {
      const existingScan = await tx.deliveryScan.findUnique({ where: { barcode } });
      if (existingScan) return { success: false, message: `Barcode ${barcode} sudah pernah di-scan` };
    }
    ...
    const isComplete = updatedItems.every((i: any) => {
...
export async function refreshDelivery(date: string, shiftKey: string, slotId: number, initialItems: Array<{material: string, required: number}> = []) {
  const compositeKey = `${date}_${shiftKey}_${slotId}`;
  ...
  if (existing && existing.status === "preparing") {
    const hasScans = existing.items.some((i: any) => parseFloat(String(i.scannedPallets || "0")) > 0);
    if (!hasScans) {
...
export async function isBarcodeInActiveDelivery(barcode: string) {

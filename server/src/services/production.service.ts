...
export async function getLineStockByLine(line: string) { return db.lineStock.findMany({ where: { line } }); }
export async function getLineBarcodes(line?: string) {
  if (line) return db.lineBarcode.findMany({ where: { line } });
  return db.lineBarcode.findMany();
}

export async function receiveToLine(line: string, material: string, barcode: string, inputPcs: number) {
  if (!line || !material || !barcode) return { success: false, message: "Line, material, barcode harus diisi." };

  return db.$transaction(async (tx: any) => {

    const inv = await tx.transitInventory.findFirst({ where: { barcode, materialName: material } });
    if (!inv) return { success: false, message: `Barcode ${barcode} tidak di transit untuk ${material}.` };
    ...
    if (!takeResult.success) return takeResult;
...
export async function receivePartialToLine(line: string, material: string, barcode: string, partialPcs: number) {
  if (!line || !material || !barcode || partialPcs <= 0) return { success: false, message: "Line, material, barcode, dan qty harus valid." };
  ...
  return db.$transaction(async (tx: any) => {
    ...
    if (inv.pcs) {
      ...
      if (lastInMutation) availablePcs = parseFloat(String(lastInMutation.qty));
    }
...
export async function returnFromLine(barcode: string, pcsOverride?: number, targetBlockRowId?: string, condition: string = "utuh") {
  if (!barcode) return { success: false, message: "Barcode harus diisi." };

  return db.$transaction(async (tx: any) => {
    const bcd = await tx.lineBarcode.findFirst({ where: { barcode } });
    if (!bcd) return { success: false, message: `Barcode ${barcode} tidak ditemukan di line.` };
    ...
    if (ls) {
      ...
      if (newQty === 0 && newPcs === 0) await tx.lineStock.delete({ where: { id: ls.id } });
      else await tx.lineStock.update({ where: { id: ls.id }, data: { qtyPallets: newQty, pcs: String(newPcs) } });
    }
...
export async function returnSisaFromLine(line: string, materialName: string, pcs: number, targetBlockRowId?: string) {
  if (!line || !materialName || !pcs || pcs <= 0) return { success: false, message: "Line, material, dan qty harus valid." };

  return db.$transaction(async (tx: any) => {
    // Verify stock exists in line
    const ls = await tx.lineStock.findFirst({ where: { line, materialName } });
    if (!ls) return { success: false, message: `Tidak ada stok ${materialName} di Line ${line}.` };

    const currentPcs = parseFloat(String(ls.pcs || "0"));
    if (pcs > currentPcs) return { success: false, message: `Qty retur (${pcs}) melebihi stok line (${currentPcs}).` };
...
export async function verifyReturn(id: string, action: "accept" | "reject") {
  return db.$transaction(async (tx: any) => {
    const ret = await tx.pendingReturn.findUnique({ where: { id } });
    if (!ret) return { success: false, message: "Data retur tidak ditemukan." };
    ...
    if (action === "accept") {
      ...
      const res = await transitService.receiveToTransit(
        ret.materialName, 
        1, 
        ret.barcode, 
        pcs, 
        `Line ${ret.line} -> Transit (Retur${ret.condition === "sisa" ? " SISA" : ""})`, 
        ret.targetBlockRowId || undefined,
        ret.supplier || "-",
        tx
      );
      ...
      if (ls) await tx.lineStock.update({ where: { id: ls.id }, data: { qtyPallets: ls.qtyPallets + 1, pcs: String(parseFloat(String(ls.pcs || "0")) + pcs) } });
      else await tx.lineStock.create({ data: { line: ret.line, materialName: ret.materialName, qtyPallets: 1, pcs: String(pcs) } });
...
export async function getExternalOnhand(dest: string) {
...
export async function processLineReject(line: string, materialName: string, pcsInput: number, reason: string) {
  if (!line || !materialName || !pcsInput || !reason) return { success: false, message: "Lengkapi data rijek." };
  ...
  return db.$transaction(async (tx: any) => {
    ...
    if (pcsInput > actualPcs) return { success: false, message: `Gagal: Total stok ${materialName} di line ini hanya ${actualPcs} PCS` };
...
export async function verifyLineReject(id: string, action: "accept" | "reject", finalPcs?: number) {
  return db.$transaction(async (tx: any) => {
    const reject = await tx.lineReject.findUnique({ where: { id } });
    if (!reject) return { success: false, message: "Data pengajuan rijek tidak ditemukan." };
    if (reject.status !== "pending") return { success: false, message: `Verifikasi ditolak: Status sudah ${reject.status}.` };
    ...
    if (!ls) return { success: false, message: `Gagal diproses: Stok ${reject.materialName} sudah kosong di Line ${reject.line}.` };
...
export async function getLineRejects(dateStr?: string) {
...
export async function saveLineOpname(data: {
  ...
  checkedBy?: string;
  notes?: string;
  items: Array<{ materialName: string; qtyBook: number; qtyPhysical: number; calculatorNotes?: string }>;
}) {
  return db.$transaction(async (tx: any) => {
...
export async function getLineOpnames(filters?: { line?: string; type?: string; startDate?: string; endDate?: string }) {
  const where: any = {};
...
export async function updateLineOpnameItem(opnameId: string, itemId: string, newQtyPhysical: number, editedBy: string) {
  return db.$transaction(async (tx: any) => {
...
export async function getLineMutations(filters: { material?: string; startDate?: string; endDate?: string; line?: string } = {}, page: number = 1, limit: number = 50) {
  const where: any = {};

...
// ═══════════════════════════════════════════
async function sumStockFromCheck(checkDate: string): Promise<Record<string, number>> {
  ...
  const rowIds = [...new Set(entries.map((e: any) => e.blockRowId))];
  ...
  for (const rowId of rowIds) {
    ...
    const rowEntries = entries.filter((e: any) => e.blockRowId === rowId);
...
export async function getStockBalanceForDate(date: string): Promise<Record<string, number>> {
  ...
  if (Object.keys(todayStock).length > 0) return todayStock;
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (Object.keys(prevStock).length > 0) return prevStock;
  }
...
// ═══════════════════════════════════════════
async function allocateFromWMS(material: string, requiredPcs: number) {
  if (requiredPcs <= 0) return { batches: [], totalSPB: 0 };
  ...
  const batches: Array<{ supplier: string; qtyPerPallet: number; pallets: number; pcs: number }> = [];
  let totalSPB = 0;
  ...
  for (const batch of availableStock) {
    ...
    if (batch.palletsAvailable <= 0) continue;
...
export async function getMaterialRequirements(date: string) {
  ...
  for (const g of Object.values(grouped)) {
    ...
    const palletCount = allocation.batches.reduce((sum: number, b: any) => sum + b.pallets, 0);
...
// ═══════════════════════════════════════════
export async function getLineMaterialRequirements(date: string) {
  ...
  for (const s of filtered) {
    ...
    for (const comp of bom) {
      ...
      if (!agg[key]) agg[key] = { line: s.line, material: comp.materialName, sh1: 0, sh2: 0, sh3: 0, buffer: 0 };
...
export async function getHourlyDistribution(date: string) {

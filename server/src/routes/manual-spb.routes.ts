...
router.post("/:itemId/process", async (req: Request, res: Response) => {
  try {
    const { barcode, pcs, supplier, targetBlockRowId } = req.body;
    const result = await manualSpbService.processSpbItem(
      req.params.itemId,
      barcode || "-",
      pcs || 0,
      supplier || "-",
...
router.get("/receive", async (req: Request, res: Response) => {
  try {
    ...
    if (!barcode) { res.status(400).json({ error: "Barcode missing" }); return; }

    const scan = await manualSpbService.getManualSpbScanByBarcode(barcode as string);
    if (!scan) {
      res.json(null);
      return;
    }

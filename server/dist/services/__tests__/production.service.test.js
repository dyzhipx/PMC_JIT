...
describe('Production Service (Unit)', () => {
    ...
    describe('receiveToLine', () => {
        ...
        it('should fail if strict validation on inputPcs does not match expected pcs', async () => {
            ...
            prismaMock.transitInventory.findFirst.mockResolvedValue({ barcode: 'BAR123', materialName: 'MAT' });
            // Mock the strict validation query
            prismaMock.stockMutation.findFirst.mockResolvedValue({ qty: '20' });
            const result = await productionService.receiveToLine('LINE_1', 'MAT', 'BAR123', 10);
            expect(result.success).toBe(false);
        ...
        it('should succeed if all checks pass and transit take is successful', async () => {
            prismaMock.transitInventory.findFirst.mockResolvedValue({ barcode: 'BAR123', supplier: 'SupA' });
            prismaMock.stockMutation.findFirst.mockResolvedValue({ qty: '10' }); // Strict validation matches 10
            // Mock the transit deductor
            vi.mocked(transitService.takeFromTransit).mockResolvedValue({ success: true });

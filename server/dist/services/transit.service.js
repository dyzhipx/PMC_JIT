...
export async function getTransitInfo() {
    ...
    const info = { blocks: [], materials: {}, materialsPcs: {} };
    for (const block of layout) {
        const bInfo = { id: block.id, blockNumber: block.blockNumber, rows: [] };
        for (const row of block.rows) {
            const rowLive = liveStock.filter((l) => l.blockRowId === row.id);
 ...
 */
async function repairTransitData(ids) {
    ...
    for (const item of itemsList) {
        ...
        if (item.barcode && item.barcode !== "-") {
            ...
            if (wRef) {
                if (resolvedSupplier === "-" && wRef.supplierName)
                    resolvedSupplier = wRef.supplierName;
                if (resolvedMid === "-" && wRef.mid)
                    resolvedMid = wRef.mid;
            }
        }
        ...
        if (resolvedSupplier === "-" || resolvedMid === "-") {
            ...
            if (warehouseRef) {
                if (resolvedSupplier === "-" && warehouseRef.supplierName)
                    resolvedSupplier = warehouseRef.supplierName;
                if (resolvedMid === "-" && warehouseRef.mid)
                    resolvedMid = warehouseRef.mid;
            }
        }
        ...
        if (resolvedSupplier === "-" || resolvedMid === "-") {
            ...
            if (otherTransit) {
                if (resolvedSupplier === "-" && otherTransit.supplier && otherTransit.supplier !== "-")
                    resolvedSupplier = otherTransit.supplier;
                if (resolvedMid === "-" && otherTransit.mid && otherTransit.mid !== "-")
                    resolvedMid = otherTransit.mid;
            }
        }
        if (resolvedSupplier !== item.supplier || resolvedMid !== item.mid) {
            const updateData = {};
            if (resolvedSupplier !== item.supplier)
                updateData.supplier = resolvedSupplier;
            if (resolvedMid !== item.mid)
                updateData.mid = resolvedMid;
...
export async function receiveToTransit(material, qtyPallet, barcode, actualPcs, source = "Gudang -> Transit", targetBlockRowId, supplier = "-", tx, midOverride, dateInGudangOverride) {
    const client = tx || db;
    ...
    for (const block of layout) {
        for (const row of block.rows) {
            if (row.id === targetBlockRowId) {
                const live = liveStock.find((l) => l.blockRowId === row.id);
    ...
    if (!target) {
        if (candidates.length === 0)
            return { success: false, message: `Tidak ada blok untuk ${material}.` };
        candidates.sort((a, b) => a.qty - b.qty);
        target = candidates.find((c) => c.qty + qtyPallet <= c.maxPallets);
        if (!target) {
    ...
    if (target.liveId) {
        const live = liveStock.find((l) => l.id === target.liveId);
    ...
    if ((resolvedMid === "-" || !resolvedDateInGudang) && barcode && barcode !== "-") {
        try {
            ...
            if (warehouseRef) {
                if (resolvedMid === "-" && warehouseRef.mid)
                    resolvedMid = warehouseRef.mid;
                if (!resolvedDateInGudang && warehouseRef.dateIn)
                    resolvedDateInGudang = warehouseRef.dateIn;
            }
        }
        catch (_err) {
    ...
    if (source.includes("Retur SISA"))
        sourceCat = "RETURN_PARTIAL";
    else if (source.includes("Retur UTUH"))
        sourceCat = "RETURN_FULL";
    else if (source.includes("Ditolak dari"))
        sourceCat = "REJECTED_OUTBOUND";
...
export async function takeFromTransit(material, qty, line, forcedPcs, tx, barcode) {
    const client = tx || db;
    const uom = await masterService.getMaterialUOM(material, client);
        ...
        if (inv) {
            ...
            if (inv.blockRowId) {
                ...
                if (live) {
                    ...
                    // Record mutation
                    const row = allRows.find((r) => r.id === inv.blockRowId);
                    if (row) {
                        const timeInStr = nowTimeStr();
                        await client.stockMutation.create({
                            data: {
    ...
    if (remaining > 0) {
        ...
        for (const stock of liveStock) {
            ...
            const row = allRows.find((r) => r.id === stock.blockRowId);
            if (row) {
                const timeInStr = nowTimeStr();
                await client.stockMutation.create({
                    data: {
        ...
        if (remT > 0) {
            ...
            for (const i of details) {
                ...
                if (take >= i.palletsAvailable)
                    await client.transitInventory.delete({ where: { id: i.id } });
                else
                    await client.transitInventory.update({ where: { id: i.id }, data: { palletsAvailable: i.palletsAvailable - take } });
                remT -= take;
            }
        }
    }
...
export async function markBarcodeUsed(barcode, tx) {
    const client = tx || db;
...
export async function isBarcodeUsed(barcode) {
...
export async function getStockCheck(date) {
...
export async function saveStockCheck(date, entries, checkedBy) {
    ...
    if (date === todayStr()) {
        ...
        for (const e of entries) {
            ...
            if (isNaN(val) || val <= 0)
                continue; // Skip empty/zero pallets
            const rowDef = allRows.find(r => r.id === e.blockRowId);
            const matName = rowDef?.materialName;
            if (!matName)
                continue;
            if (!agg[e.blockRowId])
                agg[e.blockRowId] = { pcs: 0, pallets: 0, material: matName };
        ...
        for (const [rid, data] of Object.entries(agg)) {
            ...
            if (lastInv?.supplierName) {
                ...
                if (lastTransit?.supplier)
                    resolvedSupplier = lastTransit.supplier;
            }
            const timeInStr = nowTimeStr();
        ...
        const newTotalsByRow = {}; // rowId -> material -> qty
        for (const [rid, data] of Object.entries(agg)) {
            if (!newTotalsByRow[rid])
                newTotalsByRow[rid] = {};
            newTotalsByRow[rid][data.material] = (newTotalsByRow[rid][data.material] || 0) + data.pcs;
        }
        // 5. Calculate OLD totals PER ROW
        const oldTotalsByRow = {};
        for (const l of oldLive) {
            if (!l.materialName || !l.blockRowId)
                continue;
            if (!oldTotalsByRow[l.blockRowId])
                oldTotalsByRow[l.blockRowId] = {};
...
export async function getMutationReport(filters = {}, page = 1, limit = 50) {
    const where = {};
    ...
    if (filters?.blockRowId && filters.blockRowId !== "ALL")
        where.blockRowId = filters.blockRowId;
...
export async function requestTransitOutbound(barcode, destination, targetLine) {
    const inv = await db.transitInventory.findFirst({ where: { barcode } });
    if (!inv)
        return { success: false, message: `Barcode ${barcode} tidak di Transit.` };
...
export async function verifyTransitOutbound(id, action) {
...
export async function relocateTransitPallet(barcode, targetBlockRowId, user) {
    return await db.$transaction(async (client) => {
        ...
        if (!inv)
            throw new Error("Barcode tidak ditemukan di Area Transit.");
        if (inv.blockRowId === targetBlockRowId)
            throw new Error("Barcode sudah berada di blok tujuan.");
        const targetRow = await client.blockRow.findUnique({
            where: { id: targetBlockRowId },
            include: { blockLayout: true }
        });
        if (!targetRow)
            throw new Error("Baris tujuan tidak ditemukan.");
        ...
        const totalCurrentQty = rowLiveStock.reduce((sum, l) => sum + (l.qtyPallets || 0), 0);
        ...
        const liveTargetForThisMaterial = rowLiveStock.find((l) => l.materialName === inv.materialName);
        const defPQ = await masterService.getPalletQty(inv.materialName, client);
        ...
        const uom = await masterService.getMaterialUOM(inv.materialName, client);
        const timeInStr = nowTimeStr();
...
// -------------------------------------------
export async function getTransitOpnames(filters = {}) {
    const where = {};
    if (filters.blockId)
        where.blockId = filters.blockId;
...
export async function saveTransitOpname(payload) {
        ...
        for (const item of payload.items) {
            ...
            if (Math.abs(delta) > 0.0001) {
                ...
                if (live) {
                    ...
                    const defPQ = await masterService.getPalletQty(item.materialName, tx);
                ...
                const uom = await masterService.getMaterialUOM(item.materialName, tx);
...
export async function updateTransitOpnameItem(opnameId, itemId, newQtyPhysical, editedBy) {
    return await db.$transaction(async (tx) => {

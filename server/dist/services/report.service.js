...
export async function getProductionMutationReport(filters = {}) {
    const where = {};
    ...
    mutations.forEach((m) => matSet.add(m.materialName));
    ...
    lineStocks.forEach((ls) => matSet.add(ls.materialName));
    const reportList = [];
    ...
    for (const matName of Array.from(matSet)) {
        if (filters.material && filters.material !== 'ALL' && matName !== filters.material)
            continue;
        let initialStock = 0;
        let totalIn = 0;
        let totalConsume = 0;
        ...
        let totalReject = 0;
        let totalAdjust = 0;
        const matMutations = mutations.filter((m) => {
            if (m.materialName !== matName)
                return false;
            if (filters.line && filters.line !== 'ALL') {
                if (!m.line?.includes(filters.line))
                    return false;
            }
            return true;
        });
        matMutations.forEach((m) => {
            ...
            if (isAfterEnd)
                return;
            if (isBeforeStart) {
                if (m.source === 'PRODUCTION')
                    initialStock += Math.abs(q);
                else if (m.source === 'PROD_BPP' || m.type === 'CONSUME')
                    initialStock -= Math.abs(q);
                else if (m.source === 'RETURN_FULL' || m.source === 'RETURN_PARTIAL')
                    initialStock -= Math.abs(q);
                else if (m.source === 'LINE_REJECT')
                    initialStock -= Math.abs(q);
                else if (m.source === 'LINE_OPNAME' || m.type === 'ADJUST')
                    initialStock += q;
            }
            else {
                if (m.source === 'PRODUCTION')
                    ...
                    totalConsume += Math.abs(q);
                else if (m.source === 'RETURN_FULL' || m.source === 'RETURN_PARTIAL')
                    totalReturn += Math.abs(q);
                else if (m.source === 'LINE_REJECT')
                    totalReject += Math.abs(q);
                else if (m.source === 'LINE_OPNAME' || m.type === 'ADJUST')
                    totalAdjust += q;
            }
        });
        ...
        if (shouldCompareActual) {
            actualStock = 0;
            lineStocks.filter((ls) => ls.materialName === matName).forEach((ls) => {
...
export async function getTransitMutationReport(filters = {}) {
    const where = {};
    if (filters.material && filters.material !== "ALL")
        where.materialName = filters.material;
    if (filters.block && filters.block !== "ALL")
        where.blockId = filters.block;
    if (filters.row && filters.row !== "ALL")
        where.blockRowId = filters.row;
    if (filters.line && filters.line !== "ALL")
        where.line = filters.line;
    if (filters.sku && filters.sku !== "ALL")
        where.skuId = filters.sku;
    ...
    const reportEndDate = filters.endDate || null;
    const shouldCompareActual = !reportEndDate || reportEndDate >= _today;
    let transitInventories = [];
    if (shouldCompareActual) {
        const tiWhere = {};
        if (filters.material && filters.material !== "ALL")
            tiWhere.materialName = filters.material;
        if (filters.block && filters.block !== "ALL")
            tiWhere.blockId = filters.block;
        if (filters.row && filters.row !== "ALL")
            tiWhere.blockRowId = filters.row;
    ...
    for (const matName of Array.from(matSet)) {
        ...
        const materialMutations = mutations.filter((m) => m.materialName === matName);
        materialMutations.forEach((m) => {
            const q = parseFloat(String(m.qty)) || 0;
            const mutDate = getDateStr(m.date);
            const isBeforeStart = reportStartDate && mutDate < reportStartDate;
            const isAfterEnd = reportEndDate && mutDate > reportEndDate;
            ...
            if (isBeforeStart || isStockCheckBaseline) {
                ...
                if (m.type === 'IN') {
                    ...
                    if (m.source === 'RELOKASI') {
                        ...
                        totalOut += Math.abs(q);
                        if (m.source === 'PRODUCTION')
                            summary.totalProductionOut += Math.abs(q);
                    }
                }
        ...
        if (shouldCompareActual && (!filters.sku || filters.sku === 'ALL') && (!filters.line || filters.line === 'ALL')) {
            actualStock = 0;
            transitInventories.filter((ti) => ti.materialName === matName).forEach((ti) => {

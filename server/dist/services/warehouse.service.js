...
export async function consumeFromWMS(material, qtyPallet, barcode, tx) {
    const consumed = [];
    const client = tx || db;
    if (barcode && barcode !== "-") {
...
export async function ensureCounters() {
    const countBar = await db.systemCounter.findUnique({ where: { id: "barcode_counter" } });
    if (!countBar)
        await db.systemCounter.create({ data: { id: "barcode_counter", value: 0 } });
    const countMid = await db.systemCounter.findUnique({ where: { id: "mid_counter" } });
    if (!countMid)
        await db.systemCounter.create({ data: { id: "mid_counter", value: 0 } });
}
// ═══════════════════════════════════════════
//  WAREHOUSE OUTBOUND (MULTI-DESTINATION)
// ═══════════════════════════════════════════
export async function requestWarehouseOutbound(barcode, destination) {
    const inv = await db.warehouseInventory.findFirst({ where: { barcode } });
    if (!inv)
        return { success: false, message: `Barcode ${barcode} tidak ada di Gudang.` };
    if (inv.palletsAvailable <= 0)
        return { success: false, message: `Barcode ${barcode} sudah kosong.` };
